...
function dataStoreReady(dsm) {
    ...
    get_data = function() {
        var data = deletedTasks.query();
        data.sort(function(a,b){ 
            var ac = a.get('deleted');
            var bc = b.get('deleted');
            if (ac > bc) { return -1; }
            if (ac < bc) { return 1;}
            if (ac == bc) { return 0;}
        });
    ...
    kap.add_passive_command('y', function(term) {    
        ...
        for(var i = 0; i < rows.length; i++) {
            var obj = rows[i];
            var result = deletedTasks.query({'uid':obj.id});
            if (result.length) {
                var new_slug = result[0].get('slug');
                if (new_slug.search('#recovered') == -1) {
                    new_slug += ' #recovered';
                }
                add_task(new_slug);
